Aospman Test Credential Provider
================================

This project is a test-only adaptation of Android's official MyVault sample, pinned from
`android/identity-samples` commit `d9adcb9fc581800b3f44f26e974b1e6fc806ee5d` on 2026-08-19.
It exists to validate Credential Manager and WebView conditional-passkey routing on an AOSP
`userdebug` Cuttlefish image, which does not include Google Password Manager.

The provider owns a narrow test allowlist for
`com.example.webviewpasskeybrowser.browser`, pinned to debug certificate SHA-256
`D6:B8:90:EC:A6:F4:70:8A:FF:24:BE:19:17:6C:56:AF:EE:38:D8:D4:D5:61:2A:99:64:F5:F5:81:CC:C2:B4:B1`.
It does not modify, bypass, or impersonate Google Password Manager's approval policy. Do not use
this sample, its bundled debug keystore, or its demonstration credential storage in production.

## Aospman test setup

The checked-in allowlist fingerprint matches the browser APK used for the recorded Cuttlefish
experiment. If you rebuild `webview-passkey-browser` with a different debug key, replace the
fingerprint in `TestBrowserAllowlist.kt` with the value reported by:

```sh
apksigner verify --print-certs webview-passkey-browser/app/build/outputs/apk/browser/debug/app-browser-debug.apk
```

Then build and test the provider with:

```sh
./gradlew :app:assembleDebug :app:testDebugUnitTest
```

On Android 14 or newer, enable the resulting provider in the system Credential Manager settings
before running the browser experiment. The exact package, signer, WebView, AOSP, and relying-party
revisions used for the successful run are recorded under
`artifacts/webview-conditional-cuttlefish-20260819/investigation.md`.

Upstream MyVault documentation
------------------------------
